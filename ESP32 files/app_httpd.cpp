...
int vel_izq_global=255;
int vel_der_global=255;
String direccion_global="";

//TaskHandle_t Task1;

extern String WiFiAddr;

void WheelAct(int nF, int nB,int veli,int veld);
void mover_carrito();
//void metodo(void * pvParameters);
typedef struct {
...
static esp_err_t direccion_handler(httpd_req_t *req){
    char *buf=NULL;
    size_t buf_len = httpd_req_get_url_query_len(req)+1;// extra 1 byte for null termination
    buf = (char *) malloc(buf_len); 
    httpd_req_get_url_query_str(req, buf, buf_len);
    Serial.println(buf);
    direccion_global=buf;
    mover_carrito();
    httpd_resp_set_type(req, "text/html");
...
static esp_err_t vel_handler(httpd_req_t *req){
    char *buf=NULL;
    size_t buf_len = httpd_req_get_url_query_len(req)+1;// extra 1 byte for null termination
    buf = (char*) malloc(buf_len); 
    httpd_req_get_url_query_str(req, buf, buf_len);
    ...
    String vel1=buf;
    String vel2=buf;
    vel1=vel1.substring(0,3);
    vel2=vel2.substring(3,6);
    Serial.println("Vel1: "+vel1+" Vel2: "+vel2);
    vel_izq_global=vel1.toInt();
    vel_der_global=vel2.toInt();
    mover_carrito();
    httpd_resp_set_type(req, "text/html");
    const char resp[] = "URI GET Response";
    httpd_resp_send(req, resp, strlen(resp));
...
void startCameraServer(){
   ...
   httpd_uri_t stream_uri = {
    ...
    if (httpd_start(&camera_httpd, &config) == ESP_OK) {
        ...
        httpd_register_uri_handler(camera_httpd, &vel_uri);
        httpd_register_uri_handler(camera_httpd, &vel_uri);
...
//        httpd_register_uri_handler(camera_httpd, &vel1_uri);
//        httpd_register_uri_handler(camera_httpd, &vel2_uri);
//        httpd_register_uri_handler(camera_httpd, &vel3_uri);
        //Maximo 8 al parecer
        //httpd_register_uri_handler(camera_httpd, &vel4_uri);
...
void mover_carrito(){
    if(direccion_global=="s"){
      Serial.println("atras");
      WheelAct(HIGH,LOW,vel_izq_global,vel_der_global);
    }
    if(direccion_global=="w"){
      Serial.println("adelante");
      WheelAct(LOW,HIGH,vel_izq_global,vel_der_global);
    }
}
void WheelAct(int nF, int nB,int veli,int veld)
{
 digitalWrite(gpF, nF);
 digitalWrite(gpB, nB);
 ledcWrite(vel_izq,veli);
 ledcWrite(vel_der,veld);
}
